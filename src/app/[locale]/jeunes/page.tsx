  ...
  return (
    ...
    <div data-universe="jeunes">
      ...
      >
        <div className={`${WRAP} py-16`}>
          <Reveal className="max-w-[60ch]">
            ...
            <h2 className="mt-3 font-display text-[clamp(26px,3.2vw,38px)]">
              {c.parcours.title}
            </h2>
          ...
          <RevealGroup className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {c.parcours.steps.map((s) => (
              ...
              >
                <span className="grid h-9 w-9 place-items-center rounded-pill bg-accent font-mono text-sm font-semibold text-accent-contrast">
                  {s.n}
                </span>
          ...
          <RevealGroup className="mt-6 grid items-stretch gap-4 lg:grid-cols-2">
            <RevealItem className="rounded-md border border-line bg-paper p-6">
              ...
              <p className="mt-3 max-w-[46ch] text-[15.5px] leading-relaxed text-ink-soft">
                {c.parcours.gam.body}
              </p>
      ...
      <section id="programmes" className={`${WRAP} scroll-mt-20 py-16`}>
        <Reveal className="max-w-[60ch]">
          <h2 className="font-display text-[clamp(26px,3.2vw,38px)]">
            {c.programmes.title}
          </h2>
          <p className="mt-3 text-lg leading-relaxed text-ink-soft">
            {c.programmes.lead}
        ...
        <RevealGroup className="mt-8 grid gap-4 lg:grid-cols-3">
          <RevealItem className="flex flex-col rounded-md bg-accent p-6 text-accent-contrast lg:row-span-2">
            ...
            <h3 className="mt-2 font-display text-2xl text-accent-contrast">
              {c.programmes.featured.title}
            </h3>
            <p className="mt-3 leading-relaxed text-accent-contrast/90">
              {c.programmes.featured.body}
            </p>
            <a
              href="#mentorat"
              className="mt-5 inline-flex w-fit items-center justify-center rounded-sm bg-accent-contrast px-4 py-2.5 text-sm font-semibold text-accent transition-opacity hover:opacity-90"
            >
          ...
          <RevealItem className="flex flex-col rounded-md border border-line bg-surface p-6 lg:col-span-2">
            <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-accent-text">
              {c.programmes.funding.kicker}
            </span>
            <h3 className="mt-2 font-display text-xl">
              {c.programmes.funding.title}
            </h3>
            <p className="mt-2 max-w-[60ch] text-[15px] leading-relaxed text-ink-soft">
              {c.programmes.funding.body}
            </p>
            <a
              href="#rejoindre"
              className="mt-4 inline-flex w-fit items-center justify-center rounded-sm border border-line-strong px-3.5 py-2 text-[13px] font-semibold text-ink transition-colors hover:border-ink hover:bg-accent-tint"
            >
        ...
        <div className={`${WRAP} py-16`}>
          <Reveal className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <h2 className="mt-3 font-display text-[clamp(26px,3.2vw,36px)]">
                {c.mentor.title}
              </h2>
              ...
              <a
                href="#mentorat"
                className="mt-6 inline-flex items-center justify-center rounded-sm bg-accent px-[18px] py-[11px] text-sm font-semibold text-accent-contrast transition-colors hover:bg-accent-strong"
              >
                {c.mentor.cta}
              </a>
            </div>
            <ul className="flex flex-col gap-3">
              {c.mentor.mentors.map((m) => (
                <li
                  key={m.name}
                  className="flex items-center gap-3.5 rounded-md border border-line bg-paper p-4"
                >
                  <span
                    aria-hidden="true"
                    className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-accent-tint font-mono text-sm font-semibold text-accent-text"
                  >
                    {m.initials}
                  </span>
      ...
      <section className={`${WRAP} py-16`}>
        <Reveal className="mx-auto max-w-[42ch] text-center">
          ...
          <p className="mt-5 text-sm text-ink-soft">
            <b className="text-ink">{c.testimonial.attrName}</b>,{' '}
            {c.testimonial.attrPlace}{' '}
        ...
        <RevealGroup className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-4">
          {c.testimonial.stats.map((s) => (
            <RevealItem
              key={s.label}
              className="rounded-md border border-line bg-surface p-5 text-center"
            >
      ...
      <section className="bg-accent text-accent-contrast">
        <div className={`${WRAP} py-16 text-center`}>
          <Reveal>
            <h2 className="mx-auto max-w-[20ch] font-display text-[clamp(26px,3.4vw,40px)] text-accent-contrast">
              {c.cta.title}
            </h2>
            <p className="mx-auto mt-4 max-w-[52ch] leading-relaxed text-accent-contrast/90">
              {c.cta.body}
            </p>
